...
_SEARCH_RE = re.compile(
    ...
    r"what'?s the latest|latest news on|find (?:me )?(?:info|information|news) (?:on|about))\b",
    re.I,
)
...
def detect_web_intent(query: str) -> tuple[str, str] | None:
    ...
    the models this app runs isn't something to bet every message on. A URL
    in the message wins over a search phrase (more specific intent).
    Returns (kind, target) where kind is 'fetch' or 'search', or None."""
...
def status_label(kind: str, target: str) -> str:
    return f"Fetching {target}…" if kind == "fetch" else "Searching the web…"
